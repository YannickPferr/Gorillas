...
public class GorillasTestAdapterMinimal {
	...
	Entity banana;
	Entity sun;
	Vector2f gorillaLeft;
	Vector2f gorillaRight;
	int frameWidth;
	int frameHeight;
	int gorillaHeight = 37;
	int gorillaWidth = 42;
	int sunWidth = 100;
	int sunHeight = 100;
	ArrayList<Vector2f> map;

	public GorillasTestAdapterMinimal() {
		super();
		gps = new GamePlayState(TestGorillas.GAMEPLAYSTATE);
		gss = new GameSetupState(TestGorillas.GAMESETUPSTATE);
		gps.angleIn = new EditField();
		gps.speedIn = new EditField();

		gorillaL = new Entity("gorilla");
		gorillaL.setSize(new Vector2f(gorillaWidth, gorillaHeight));

		gorillaR = new Entity("gorilla2");
		gorillaR.setSize(new Vector2f(gorillaWidth, gorillaHeight));

		banana = new Entity("banana");
		banana.setSize(new Vector2f(10, 10));

		sun = new Entity("sun");
		sun.setSize(new Vector2f(sunWidth, sunHeight));
	}
	...
	public void setPlayerNames(String player1Name, String player2Name) {
		if (gorillas.getCurrentStateID() == TestGorillas.GAMESETUPSTATE) {
			GameSetupState.player1 = player1Name;
			GameSetupState.player2 = player2Name;
		}
	}
	...
	public void startGameButtonPressed() {
		// TODO: Implement
		if (gorillas.getCurrentStateID() == TestGorillas.GAMESETUPSTATE) {
			if (GameSetupState.player1 != null
					&& GameSetupState.player1.length() != 0
					&& GameSetupState.player2 != null
					&& GameSetupState.player2.length() != 0) {
				if (!GameSetupState.player1.equals(GameSetupState.player2)) {
	...
	public void fillVelocityInput(char charac) {
		...
		String input = Character.toString(charac);
		if (gps.speedIn.getText().equals("-1") && Character.isDigit(charac))
			gps.speedIn.setText("");

		if (Character.isDigit(charac)
				&& Integer.parseInt(gps.speedIn.getText() + input) <= 200
				&& Integer.parseInt(gps.speedIn.getText() + input) >= 0) {
			if (gps.speedIn.getText().isEmpty())
				gps.speedIn.setText(input);
			else
				gps.speedIn.setText(gps.speedIn.getText() + input);
		}
	}
	...
	public int getVelocityInput() {
		...
		if (gps.speedIn.getText().length() != 0)
			return Integer.parseInt(gps.speedIn.getText());
		else
			return -1;
	}
	...
	public void fillAngleInput(char charac) {
		...
		String input = Character.toString(charac);
		if (gps.angleIn.getText().equals("-1") && Character.isDigit(charac))
			gps.angleIn.setText("");

		if (Character.isDigit(charac)
				&& Integer.parseInt(gps.angleIn.getText() + input) <= 360
				&& Integer.parseInt(gps.angleIn.getText() + input) >= 0) {
			if (gps.angleIn.getText().isEmpty())
				gps.angleIn.setText(input);
			else
				gps.angleIn.setText(gps.angleIn.getText() + input);
		}
	}
	...
	public int getAngleInput() {
		...
		if (gps.angleIn.getText().length() != 0)
			return Integer.parseInt(gps.angleIn.getText());
		else
			return -1;
	}
	...
	public void resetPlayerWidget() {
		// TODO: Implement
		gps.angleIn.setText("-1");
		gps.speedIn.setText("-1");
	}
	...
	public void shootButtonPressed() {
		...
		if (!(gps.angleIn.getText().length() == 0)
			...
			if (Game.shootL) {
				...
				bananaPos = this.gorillaRight;
				enemy = this.gorillaLeft;
				xOffset = 30;
				yOffset = -38;
			}

			sun.setPosition(new Vector2f(frameWidth / 2, sunHeight / 2 + 5));

			Event loop = new Event("loop") {

				@Override
				protected boolean performAction(GameContainer gc,
						StateBasedGame sb, int delta) {

					return gps.game.inAir;
				}
			};
			loop.addAction(new Action() {
				...
				public void update(GameContainer gc, StateBasedGame sb,
					...
					gps.game.t += delta;
					banana.setPosition(getNextShotPosition(new Vector2f(
							bananaPos.x + xOffset, bananaPos.y + yOffset),
							getAngleInput(), getVelocityInput(), Game.shootL,
							(int) gps.game.t));
				}
			});
			...
			Event collision = new Event("collision") {
				...
				protected boolean performAction(GameContainer gc,
						StateBasedGame sb, int delta) {
					// TODO Auto-generated method stub
					if (banana.getPosition().x > frameWidth
							|| banana.getPosition().x < 0
							|| banana.getPosition().y > frameHeight) {

						gps.game.inAir = false;
						gps.game.t = 0;
						Game.shootL = !Game.shootL;
						return true;
					}

					for (int i = 0; i < map.size(); i++) {
						if (banana.getPosition().x - 5 >= map.get(i).x
								&& banana.getPosition().x + 5 <= map.get(i).x + 100
								&& banana.getPosition().y - 5 >= map.get(i).y
								&& banana.getPosition().y + 5 <= frameHeight) {
					...
					if (banana.getPosition().x - 5 >= enemy.x - gorillaWidth
							/ 2
							&& banana.getPosition().x + 5 <= enemy.x
									+ gorillaWidth / 2
							&& banana.getPosition().y - 5 >= enemy.y
									- gorillaHeight / 2
							&& banana.getPosition().y + 5 <= enemy.y
									+ gorillaHeight / 2) {
					...
					if (banana.getPosition().x - 5 >= sun.getPosition().x
							- sunWidth / 2
							&& banana.getPosition().x + 5 <= sun.getPosition().x
									+ sunWidth / 2
							&& banana.getPosition().y - 5 >= sun.getPosition().y
									- sunHeight / 2
							&& banana.getPosition().y + 5 <= sun.getPosition().y
									+ sunHeight / 2) {

						gps.game.sunAstonished = true;
						return false;
					}
					return false;
				}
			};
			collision.addAction(new DestroyEntityAction());
			banana.addComponent(collision);
			banana.addComponent(loop);
			gps.entityManager.addEntity(gps.stateID, banana);
		}
	}
	...
	public Vector2f getNextShotPosition(Vector2f startPosition, int angle,
		...
		float y = startPosition.y - sY
				+ (0.5f * 10 * (float) Math.pow(delta, 2));
	...
	public String getEqualError() {
		// TODO: Implement
		return "Player names must not be equal";
	}
	...
	public String getPlayer1Error() {
		...
		if (GameSetupState.player1 != null
				&& GameSetupState.player1.length() != 0
				&& !GameSetupState.player1.equals(GameSetupState.player2))
			return "";
		if (GameSetupState.player1 == null
				|| GameSetupState.player1.length() == 0)
	...
	public String getPlayer2Error() {
		// TODO: Implement
		if (GameSetupState.player2 != null
				&& GameSetupState.player2.length() != 0
				&& !GameSetupState.player1.equals(GameSetupState.player2))
			return "";
		if (GameSetupState.player2 == null
	...
	public void handleKeyPressed(int updatetime, Integer input) {
		if (gorillas != null && app != null) {
			...
			if (input == Input.KEY_N)
				gorillas.enterState(TestGorillas.GAMESETUPSTATE);
